...
for vs in virtual_systems:
    # 새로운 파일 이름 생성
    foldername = FindValue(vs, 'id') + "\\"
    print(foldername)
    os.mkdir(directory + foldername)
    filename = directory + foldername + FindValue(vs, 'id') + '.ovf'
    ...
    for disk in disklists:
        if none[idx] != 0:
            ...
            vmdkfile = FindValue(diskRefliststs[idx], 'href')
            vmdkpath = directory + vmdkfile
            shutil.move(vmdkpath, directory + foldername + vmdkfile)
            print("VMDK FILE : ", vmdkpath)
    ...
    nvramfile = FindValue(nvram, 'href')
    nvrampath = directory + nvramfile
    shutil.move(nvrampath, directory + foldername + nvramfile)
    print("NVRAM FILE : ", nvrampath)
